...
contract NftManagerTest is FishcakeTestHelperTest {
    ...
    function test_createNFT_type1() public {
        ...
        console.log(
            "NftManagerTest test_createNFT_type1 redemptionPoolAddress:",
            address(tempNftManager.redemptionPoolAddress())
        );
        ...
        tempNftManager.createNFT(
            temp_businessName, temp_description, temp_imgUrl, temp_businessAddress, temp_website, temp_social, temp_type
        );
        vm.stopBroadcast();
        ...
        assertTrue(
            before_tempNftManager_fcc - tempNftManager.proMineAmt() == after_tempNftManager_fcc,
            "before_tempNftManager_fcc - tempNftManager.proMineAmt() == after_tempNftManager_fcc"
        );
        ...
        assertTrue(
            tempNftManager.proMineAmt() == after_deployerAddress_fcc,
            "tempNftManager.proMineAmt() == after_deployerAddress_fcc"
        );
        ...
        assertTrue(
            tempNftManager.merchantValue() * 25 / 100 == after_tempNftManager_usdt,
            "tempNftManager.merchantValue() * 25 / 100 == after_tempNftManager_usdt"
        );
        ...
        assertTrue(
            before_deployerAddress_usdt - tempNftManager.merchantValue() == after_deployerAddress_usdt,
            "before_deployerAddress_usdt - tempNftManager.merchantValue() == after_deployerAddress_usdt"
        );
        ...
        assertTrue(
            tempNftManager.merchantValue() * 75 / 100 == after_redemptionPool_usdt,
            "tempNftManager.merchantValue() * 75 / 100 == after_redemptionPool_usdt"
        );
