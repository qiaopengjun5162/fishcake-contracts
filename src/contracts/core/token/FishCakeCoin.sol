    ...
    FishCakeCoinStorage
{
    ...
    modifier onlyRedemptionPool() {
        require(
            msg.sender == RedemptionPool, "FishCakeCoin onlyRedemptionPool: Only RedemptionPool can call this function"
        );
        _;
    }
    ...
    function _beforePoolAddress(fishCakePool memory _pool) internal virtual {
        ...
        require(
            _pool.directSalePool != address(0), "FishCakeCoin _beforeAllocation:Missing allocate DirectSalePool address"
        );
        require(
        ...
        require(
            _pool.ecosystemPool != address(0), "FishCakeCoin _beforeAllocation:Missing allocate EcosystemPool address"
        );
        require(
            _pool.foundationPool != address(0), "FishCakeCoin _beforeAllocation:Missing allocate FoundationPool address"
        );
    }
}
